...
description: "根据文本提示词和可选图片参考素材创建异步图片生成任务，参考素材支持文件 ID 或 HTTPS URL。"
...
<Tabs sync={false}>
  <Tab title="Option 1">
    ...
    <ParamField body="input_references" type="object[]">
      可选参考图片。可将 `POST /openapi/v1/files` 返回的 `id` 填入 `file_id`，或通过 `url` 直接传入图片的公网 HTTPS 地址。仅支持 JPG、JPEG、PNG 和 WEBP 图片，每张默认最多 10 MiB。图片须通过内容审核。

      数组总长度：`0 - 6` 个元素。可混用文件 ID 和 URL，素材顺序保持不变。每个元素必须且只能提供 `file_id`、`url` 中的一个。
      ...
      <Expandable title="子属性" defaultOpen>
        ...
        <ParamField body="file_id" pre={["input_references."]} type="string&lt;uuid&gt;">
          文件上传接口返回的当前账户文件 ID。不提供 `url` 时必填，不能同时提供这两个字段。
        </ParamField>
    ...
    <ResponseField name="error" type="object" required>
      ...
      <Expandable title="子属性" defaultOpen>
        <ResponseField name="code" pre={["error."]} type="string" required>
          ...
          - `reference_too_large`：图片超过大小限制，默认上限为 10 MiB。
        </ResponseField>
