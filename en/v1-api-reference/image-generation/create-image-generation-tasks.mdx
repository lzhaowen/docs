...
description: "Creates asynchronous image generation tasks from a prompt and optional image references supplied by file ID or HTTPS URL."
...
<Tabs sync={false}>
  <Tab title="Option 1">
    ...
    <ParamField body="input_references" type="object[]">
      Optional reference images. Set `file_id` to the `id` returned by `POST /openapi/v1/files`, or provide the image's public HTTPS URL directly in `url`. Only JPG, JPEG, PNG, and WEBP images are supported, up to 10 MiB per image by default. Images must pass content moderation.

      Allowed array length: `0 - 6` elements in total. You can mix file IDs and URLs; reference order is preserved. Each item must contain exactly one of `file_id` or `url`.
      ...
      <Expandable title="Child attributes" defaultOpen>
        ...
        <ParamField body="file_id" pre={["input_references."]} type="string&lt;uuid&gt;">
          File ID returned by the file upload endpoint for your account. Required when `url` is omitted; do not provide both fields.
        </ParamField>
    ...
    <ResponseField name="error" type="object" required>
      ...
      <Expandable title="Child attributes" defaultOpen>
        <ResponseField name="code" pre={["error."]} type="string" required>
          ...
          - `reference_too_large`: The image exceeds the size limit, which defaults to 10 MiB.
        </ResponseField>
