...
description: "Returns task status and output by batch task ID or child image or video UUID. Provide exactly one of the id or uuid query parameters."
...
<ParamField query="uuid" type="string&lt;uuid&gt;">
  Child image or video UUID returned in the generation response. Provide this or id, but not both.
...
<Tabs sync={false}>
  <Tab title="200">
    ...
    <ResponseField name="output" type="object" required>
      Current child image or video task output, including entries that are still queued or processing.
    ...
    <ResponseField name="mode" type="enum&lt;string&gt;" required>
      Image or video generation mode used by the task.

    Available options: `text_to_image`, `image_to_image`, `text_to_video`, `first_frame_image_to_video`, `first_last_frame_image_to_video`, `reference_to_video`.
    </ResponseField>

    <ResponseField name="model" type="enum&lt;string&gt; | null">
      Image or video generation model used by the task.

    Available options: `openai/gpt-image-2`, `google/nano-banana-pro`, `bytedance/seedream-4.5`, `black-forest-labs/flux-2-pro`, `bytedance/seedance-2.0`, `bytedance/seedance-2.0-fast`, `google/veo-3.1`, `xai/grok-imagine`.
    </ResponseField>
