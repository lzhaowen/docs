...
<Info>
  API v1 requires an active paid subscription. Free accounts receive a `403 subscription_required` response for every API v1 endpoint.
</Info>
...
If your reference file already has a public HTTPS URL, you can skip this upload and pass `url` directly in a generation request. Each reference must contain exactly one of `file_id` or `url`.
